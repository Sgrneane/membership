...
def is_superadmin_role(user,request):
    if user.role == roles.ADMIN and user.is_verified == True:
        return True
    else:
...
def is_admin_role(user,request):
    if user.role == roles.ADMIN and user.is_verified == True:
        ...
        messages.error(request,"You have not Authorized or You are not verified , please login and go through verify pin")
        return False

def is_all_role(user,request):
    if user.role == roles.ADMIN and user.is_verified == True:
        return True
    elif user.role==roles.USER and user.is_verified == True:
        return True
    else:
        messages.error(request,"You have not Authorized or You are not verified , please login and go through verify pin")
        return False
...
def is_USER_role(user,request):
    if user.role == roles.USER and user.is_verified == True:
        return True    
    else:
        messages.error(request,"You have not Authorized or You are not verified , please login and go through verify pin")
        return False
